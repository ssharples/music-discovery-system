"""
Enhanced PydanticAI Agent Base - Demonstrates advanced features for music discovery agents
"""
import asyncio
import logging
...
class EnhancedPydanticAgent(Generic[OutputT]):
    ...
    async def run_with_retry(
        ...
        **kwargs
        ...
        async def retry_handler(ctx: RunContext[AgentContext], exception: Exception) -> ModelRetry:
            if isinstance(exception, ValidationError):
                logger.warning(f"Validation error, retrying with clarification: {exception}")
                return ModelRetry(
                    content=f"Please ensure your response follows the exact schema. Error: {exception}"
                )
            elif "rate_limit" in str(exception).lower():
                logger.warning("Rate limit hit, waiting before retry...")
                await asyncio.sleep(5)  # Wait 5 seconds
                return ModelRetry(
                    content="Please continue with your previous response."
                )
            else:
