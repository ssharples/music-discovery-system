...
class DiscoveryOrchestrator:
    ...
    async def discover_emerging_talent_via_search(self, 
                                                 genres: List[str] = None, 
                                                 max_results: int = 20) -> List[Dict[str, Any]]:
        """
        NEW: Discover emerging talent using Firecrawl Search API
        """
        logger.info(f"🔍 SEARCH DISCOVERY: Starting emerging talent discovery via search")
        
        if not genres:
            genres = ["indie", "electronic", "hip hop", "rock", "pop", "alternative"]
        
        try:
            # Use V2 enrichment agent's search capabilities
            if self.use_v2_enrichment and hasattr(self.enrichment_agent, '_discover_emerging_talent_via_search'):
                discovered_artists = await self.enrichment_agent._discover_emerging_talent_via_search(genres)
                
                # Filter and process discovered artists
                processed_artists = []
                for artist_data in discovered_artists[:max_results]:
                    # Check if already exists in database
                    existing_artist = await self._check_artist_exists(artist_data["name"])
                    if not existing_artist:
                        processed_artists.append({
                            "name": artist_data["name"],
                            "discovery_method": "search_api",
                            "source_url": artist_data.get("source_url"),
                            "discovery_context": artist_data.get("discovery_context"),
                            "discovery_score": 0.7,  # Medium confidence for search discoveries
                            "metadata": {
                                "search_based": True,
                                "genres": genres,
                ...
                logger.info(f"🔍 SEARCH DISCOVERY: Found {len(processed_artists)} new emerging artists")
                return processed_artists
            
            else:
                logger.warning("🔍 SEARCH DISCOVERY: V2 enrichment agent not available")
                return []
                
        except Exception as e:
            logger.error(f"🔍 SEARCH DISCOVERY: Failed to discover emerging talent: {str(e)}")
            return []

    async def _check_artist_exists(self, artist_name: str) -> bool:
        """Check if artist already exists in database"""
        try:
            response = self.supabase.table("artists").select("id").eq("name", artist_name).execute()
